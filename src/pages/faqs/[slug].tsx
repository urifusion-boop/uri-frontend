...
export default function FaqPage({ faq }: { faq: any }) {
  ...
  const contentSections = Array.from(doc.querySelectorAll('ul > li')).map((li) => {
    const key = li.querySelector('strong')?.textContent || '';

    let description = li.innerHTML.replace(`<strong>${key}</strong>`, '').trim();
    ...
    <>
      ...
      <div className="bg-[#FFFCFE] min-h-screen">
        ...
        <div className="px-4 mt-[60px] md:mt-[120px] max-w-[1230px] mx-auto">
          <h3 className="text-[#141416] font-[500] md:text-[24px] text-lg">{faq.keyword}</h3>

          <ul className="md:mt-[60px] mt-[40px] md:space-y-[64px] space-y-8 md:mb-[400px] mb-[200px]">
            {contentSections.map((section, index) => (
              <li key={index} className="border border-[#7676764D]/50 border-b-[#CD1B78] p-6 rounded-lg flex gap-3 items-start">
                <HelpQuestionIcon />
                <div className="w-full">
                  <div className="flex justify-between items-center cursor-pointer" onClick={() => setActiveIndex(activeIndex === index ? null : index)}>
                    <Typography className="text-[#242A2D] font-[500] md:text-[24px] text-lg">{section.key}</Typography>
