    ...
    const filtered = faqs
      .map((faq: any) => {
        ...
        const keywordMatch = queryWords.filter((word) =>
          faq.keyword.toLowerCase().includes(word)
        ).length;

        const contentMatch = queryWords.filter((word) =>
          faq.content.toLowerCase().includes(word)
        ).length;
        ...
        <ul className="flex mb-4 flex-col px-3 md:px-4 items-center md:mt-[72px] mt-[60px] md:gap-[30px] gap-4">
          {filteredFaqs
            ...
            .map((faq: any) => (
              <li
                key={faq.slug}
                className="w-full bg-white py-[16px] md:px-4 px-2 rounded-[8px] shadow-md max-w-[967px] mx-auto"
              >
                <Link
                  href={`/faqs/${faq.slug}`}
                  className="flex items-center md:gap-[24px] gap-[16px]"
                >
                  {faq.image && (
                    <div className="md:p-[10px] p-[8px] bg-[#CD1B78] rounded-md">
                      <img
                        src={faq.image}
                        alt={faq.keyword}
                        className="object-contain md:w-[40px] w-[32px] md:h-[40px] h-[32px]"
                      />
                    </div>
                  )}
                  <p className="md:text-[24px] text-[16px] font-semibold text-[#363636] m-0 font-urbanist">
                    {faq.keyword}
                  </p>
                </Link>
              </li>
