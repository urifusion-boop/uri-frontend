...
const OrganizationLeadForm = () => {
  ...
  useEffect(() => {
    ...
    if (existingForm && isSuccess && userId && !isCreateMode) {
      ...
      // Extract Location Intelligence fields (using type assertion for optional fields)
      const { enable_location_intelligence, location_zone_center_lat, location_zone_center_lng, location_zone_radius_km, location_zone_name, min_trust_score } = existingForm as any;
      ...
      // Restore selectedPlace if we have location data
      if (location_zone_center_lat && location_zone_center_lng && location_zone_name) {
        setSelectedPlace({
          formatted_address: location_zone_name,
          latitude: location_zone_center_lat,
          longitude: location_zone_center_lng,
          place_id: '',
                ...
                <Box sx={{ mt: 3 }}>
                  <Box sx={{ mb: 3 }}>
                    <GooglePlacesAutocomplete
                      ...
                      label="Target Zone Center"
                      placeholder="Search for a city, area, or landmark (e.g., Ogba, Lagos)"
