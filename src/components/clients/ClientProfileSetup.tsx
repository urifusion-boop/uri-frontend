...
import LocationProfileSetup from './LocationProfileSetup';
...
const ClientProfileSetup = () => {
  ...
  const { formDetails, setFormDetails, createUserProfile } = useClientProfileSetupHook();
  ...
  return (
    ...
    >
        ...
        <Grid container>
          ...
          <Grid xs={12} md={8} lg={6} item>
            <Box sx={{ padding: '0px 20px' }}>
              <BusinessDetailsForm active={stage === stages[0]} formDetails={formDetails} setFormDetails={setFormDetails} setStage={(index: number) => setStage(stages[index])} />
      ...
      <CustomModal width="756px" open={open} setOpen={setOpen} closeOnOverlayClick={false}>
        <ModalContent onClickComplete={() => closeModal()} steps="two" />
      </CustomModal>
    </Box>
  );
};
